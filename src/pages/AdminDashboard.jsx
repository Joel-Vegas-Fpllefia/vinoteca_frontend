...
const AdminDashboard = () => {
  ...
  const [tab, setTab] = useState('vinos'); // Para alternar entre listas
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setProductos({ vinos: resVinos.data, cervezas: resCervezas.data });
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
      <header className="flex justify-between items-center mb-10">
        ...
        <button 
          ...
          onClick={() => alert("Aquí abriríamos el formulario de creación")}
        >
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <tbody>
            {listaActual.map((prod) => (
              <tr key={prod._id} className="border-b hover:bg-gray-50 transition-colors">
                ...
                <td className="p-4 font-mono">{prod.price}€</td>
                <td className="p-4">
        ...
        {listaActual.length === 0 && (
          <div className="p-20 text-center text-gray-400 italic">No hay productos en esta categoría.</div>
        )}
      </div>
    </div>
  );
